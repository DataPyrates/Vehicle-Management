...
$sql ='select b.*,c.first_name,c.last_name,c.company,c.phone,c.email,v.rego,v.make,v.model,v.colour,v.body_type,v.vin,v.engine_number,v.chassis_no	 from vsms_customer_booking b join vsms_customer c on b.customer = c.id join vsms_vehicle v on b.vehicle = v.id where b.id='.$_REQUEST['id'];
$statement = $db->prepare($sql);
$statement->execute(); 
$inv_data = $statement->fetchAll();

$sql1='select b.*,p.item_code from vsms_customer_booking_product b join vsms_product p on b.product = p.id where b.customer_booking_id='.$_REQUEST['id'];
...
<body id="print">
    ...
    text-decoration: uppercase;letter-spacing: 20px;padding: 8px 0px;">INVOICE</textarea>
      ...
      <div id="customer">
        ...
        <tr>
      	<td class="meta-head">Name</td><td><?php if(!empty($inv_data[0]['first_name'])) { echo $inv_data[0]['first_name'].' '.$inv_data[0][
      		'last_name']; } else { echo $inv_data[0]['company']; }?></td>
      	</tr>
        <tr>
